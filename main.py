...
#for t in tables:
#    table = t['name']
#    db.execute("drop table if exists :table",table=table)
...
def start():
    ...
    if request.method == "POST":
        ...
        db.execute("create table :slides (slide VARCHAR(50))", slides=user_slides)
        for value in request.form:
            if value != 'selectall':
                db.execute("insert into :slides (slide) VALUES (:value)", slides=user_slides, value=value+str(user_id))
        ...
        #tables = db.execute("SELECT name FROM sqlite_master WHERE type='table' and name LIKE :user_id", user_id=user_id)
        #tables.pop(0)
        #for t in tables:
        #    table = t['name']
        #    db.execute("drop table if exists :table",table=table)
...
def quiz():
    ...
    get_slides = db.execute("select * from :slides where slide like :user", slides=user_slides, user=str_user_id)
    ...
    tables = db.execute("SELECT name FROM sqlite_master WHERE type='table' and name like '%.html%' and name like :user", user=str_user_id)
    ...
    db.execute("drop table if exists :table",table=user_slide)
    db.execute("create table :table (correct VARCHAR(50), user_input VARCHAR(50), is_correct VARCHAR(20), color VARCHAR(20))", table=user_slide)
    ...
    for a in answers:
        if a.lower() == answers[a].lower():
            ...
            color = '#800000'
        db.execute("insert into :table (correct, user_input, is_correct, color) VALUES (:correct, :user_input, :is_correct, :color)",
    ...
    print(img_id)
    answers_to_score = db.execute("select * from :table", table=user_slide)
...
def final_score():
    ...
    for t in tables:
        table = t['name']
        ans = db.execute("select * from :table", table=table)
        total += len(ans)
        #print(ans)
        for a in ans:
    ...
    tables = db.execute("SELECT name FROM sqlite_master WHERE type='table' and name like :user", user=str_user_id)
    tables.pop(0)
    for t in tables:
            table = t['name']
            db.execute("drop table if exists :table",table=table)
